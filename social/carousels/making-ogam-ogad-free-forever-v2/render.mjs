...
const { chromium } = require(path.join(offGridRoot, 'console/node_modules/playwright/index.js'));
...
for (const deck of ['linkedin', 'x']) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if (overflow.horizontal || overflow.vertical) {
      throw new Error(`${deck} slide ${number} overflows: ${JSON.stringify(overflow)}`);
    }
