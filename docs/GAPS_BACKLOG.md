...
- **The dedicated OGAP success route is prepared but is not the live RevenueCat redirect yet.** `/ogap/thank-you/` must be deployed before the OGAP Web Purchase Link can safely change from `/thank-you/`. After deployment, update the RevenueCat redirect and verify one sandbox purchase from an existing Pro customer reaches the hardware confirmation page.
...
- **Platform availability was not represented consistently across the main customer journey** (resolved 2026-07-17; refreshed 2026-07-28). Home, Download, Desktop, Desktop Releases, Mobile, Mobile Releases, analytics metadata, site metadata, and `llms.txt` now present iOS, Android, macOS, and Windows. The copy distinguishes stable releases from the optional nightly tracks and records macOS and Windows x64 stable `v0.0.42`; Windows remains the free core app without Pro support. Verified the non-prerelease `v0.0.42` GitHub release, its permanent macOS and Windows installers, and the permanent Windows nightly installer; each installer URL redirects to a live release asset.
