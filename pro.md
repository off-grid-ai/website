...
- **Or ${{ site.data.pricing.price | times: site.data.pricing.lifetime_multiplier }} once, lifetime.** {{ site.data.pricing.lifetime_multiplier }} years up front and it's yours forever — every release, no renewal. Lifetime climbs with the annual price, so it's cheapest today too.
